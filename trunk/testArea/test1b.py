...
import fcntl, os, random, re, signal, struct, sys, time, types
import curses.ascii
...
def construct_packet(packet_in_string):
    ...
    try:
        ...
        raise ValueError(str(_EE122Reply.message_map[type]) + ':' + e)
...
def launch_client():
    client = Popen([c_bin, '-s', '127.0.0.1', '-p', str(s_port), '-v'],
...
def ee122_test_fault_message_type(server, clients):
    ...
    for packet in packets:
        if packet.__class__.__name__ == _EE122Reply.SpeakNotify.__name__:
            raise RuntimeError('Got speak message despite an invalid msg type')
...
if __name__ == '__main__':
    ...
    print '* Version:', _version_, '. NOT for final grading!'
    print '* TAs may add more tests or remove some tests for final grading.'
    print
    ...
    for (test, type, max, num_clients) in tests:
        ...
        try:
            ...
            print 'Failed to launch server and client(s): ', e
    ...
    # XXX: the final points formula is hidden to the students.
    print 'Your final points: not disclosed....'
