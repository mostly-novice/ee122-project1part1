...
int main(int argc, char* argv[]){
  ...
  while(!done){
	...
	case MOVE_NOTIFY:
	  ...
	  vector<struct Player*>::iterator vector_iter;

	  vector_iter = others.begin();
	  while(vector_iter != others.end())
	  {
		  /* do stuff */
		  if(strcmp(vector_iter->name,mn->name)==0){
			  found = true;
			  break;
		  }
	...
	case ATTACK_NOTIFY:
	  ...
	  vector_iter = others.begin();
	  bool attacker_in_sight  = false; // if the old position is in sight
	  bool victim_in_sight  = false; // if the new position is in sight

	  while(vector_iter != others.end())
	  {
		if(strcmp(an->attacker_name,vector_iter->name)==0)
		{
			attacker_in_sight = (abs((self->x - vector_iter->x))<=5 && abs((self->y - vector_iter->y))<=5);
			break;
		}else if(strcmp(an->victim_name,vector_iter->name)==0)
		{
			victim_in_sight = abs((self->x - p->x))<5 && abs((self->y - p->y))<5;
			break;
		}
	  }

	  if(attacker_in_sight && victim_in_sight){
		  // check if (after being damaged by an->damage) the victim's hp is not 0..print:
		  // .<attackername> damaged <victimname> by <damage>. <victim name>.sHPisnow <HP>..
