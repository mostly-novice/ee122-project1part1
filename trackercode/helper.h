...
unsigned int send_storage_location_request(char* name, unsigned int IP, unsigned short port) {
    struct header *hdr = (struct header *) malloc(sizeof(int));
    struct storage_location_request *slr = (storage_location_request *) malloc (sizeof storage_location_request);
    hdr->len = htons(0x14); // 16 + 4 = 20
    hdr->msgtype = STORAGE_LOCATION_REQUEST; // TODO: Add STORAGE_LOCATION_REQUEST to constants.h
    slr->message_type = 0x0;
    slr->ID = unique_identifier; // TODO: create unique_identifier
    strcpy(name,slr->name);
}
