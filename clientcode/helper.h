...
int sendsarequest(char x, char y, int udpsock,struct sockaddr_in * sin, int currentID, message_record * tobeack){
  ...
  tobeack->message = tosent;
  int sent_bytes = sendto(udpsock,tosent,SERVER_AREA_REQUEST_SIZE,0,(struct sockaddr*)sin,sizeof(*sin));
  if(sent_bytes < 0) {
...
int sendssrequest(Player * self, int udpsock,struct sockaddr_in * sin, int currentID, message_record * tobeack){
  ...
  int sent_bytes = sendto(udpsock,tosent,SAVE_STATE_REQUEST_SIZE,0,(struct sockaddr*)sin,sizeof(*sin));
